...
> ملاحظة إضافية: أثناء عملية التثبيت على جهاز فعلي، إذا لم يكن ملف النواة موجودًا داخل `iso/boot/`، فسيحاول سكربت التثبيت `scripts/install-to-disk.sh` استدعاء `scripts/ensure_kernel.sh` لِتنزيل وبناء النواة تلقائيًا (يحتاج إلى أدوات مثل `make` و `gcc` على الجهاز). إذا لم تكن أدوات البناء متاحة على الجهاز الهدف، فستحتاج لبناء النواة مسبقًا على آلة تطوير ونسخ `vmlinuz` إلى `iso/boot/` قبل التشغيل.
