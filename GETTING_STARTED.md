...
(ملاحظة: هذا المشروع يتضمن صورة نواة مضمّنة للمعمارية الافتراضية `x86_64` داخل `iso/boot/` لذلك يمكن تثبيت النظام على جهاز حقيقي بدون إنترنت.)

> ملاحظة إضافية: أثناء عملية التثبيت على جهاز فعلي، لن يتم تنزيل النواة تلقائيًا من الإنترنت. إذا لم تجد النواة داخل `iso/boot/`، فـ `scripts/ensure_kernel.sh` سيحاول بناء النواة محليًا فقط إذا كانت مصادر النواة موجودة في `kernel/linux-6.6` (ولا يستخدم الشبكة). إذا لم تكن أدوات البناء متاحة على الجهاز الهدف أو لم تكن المصادر موجودة، يرجى بناء النواة على جهاز تطوير ثم نسخ الملف الناتج `vmlinuz` إلى `iso/boot/` قبل محاولة التثبيت على الجهاز الهدف.
