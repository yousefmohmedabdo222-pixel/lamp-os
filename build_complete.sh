#!/bin/bash
echo "🔨 Building Complete Lamp OS with GUI"
echo "======================================"

# 0. تنظيف (اختياري)
read -p "Clean previous build? (y/n): " clean
if [[ $clean == "y" ]]; then
    rm -rf kernel gui iso initrd lamp-os-*.iso
fi

# 1. إنشاء هيكل المشروع
mkdir -p {kernel,gui,iso/{boot/grub,live},initrd,src}
...
# إعدادات مبسطة للنواة
make defconfig
echo "CONFIG_LOCALVERSION=\"-lamp\"" >> .config
...
# 3. بناء واجهة GUI متقدمة
cd gui
echo "🎨 Building advanced GUI..."
...
# بناء التطبيق
qmake lamp-desktop.pro
make -j$(nproc)

cd ..
echo "✅ GUI built"

# 4. إنشاء initrd مع GUI
cd initrd
mkdir -p opt/lamp-gui
cp ../gui/lamp-desktop opt/lamp-gui/
...
echo "========================================"
echo "         🪔 Lamp OS v1.0 GUI"
echo "========================================"

# بدء GUI
if [ -f /opt/lamp-gui/lamp-desktop ]; then
    echo "Starting Lamp OS Desktop..."
    /opt/lamp-gui/lamp-desktop &
else
    echo "GUI not found, starting shell..."
fi
...
echo ""
echo "🎉 LAMP OS COMPLETE BUILD FINISHED!"
echo "=================================="
echo "📦 Output: lamp-os-complete.iso ($(du -h lamp-os-complete.iso | cut -f1))"
echo ""
echo "🚀 Test with:"
echo "   qemu-system-x86_64 -cdrom lamp-os-complete.iso -m 2G -vga std"
echo ""
echo "💾 Save to GitHub:"
echo "   git add . && git commit -m 'Lamp OS Complete with GUI' && git push"
