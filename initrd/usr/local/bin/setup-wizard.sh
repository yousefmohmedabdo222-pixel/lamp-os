...
start_installation() {
    ...
    # Extract initrd (full filesystem)
    echo "Extracting system files..."
    INITRD_PATH="/cdrom/boot/initrd.img"
    if [ ! -f "$INITRD_PATH" ]; then
        INITRD_PATH="/run/live/medium/boot/initrd.img"
    fi
    
    if [ -f "$INITRD_PATH" ]; then
        sudo bash -c "gzip -dc '$INITRD_PATH' | (cd '$MNT' && cpio -id --no-absolute-filenames 2>/dev/null)"
        echo -ne "${GREEN}✓ System files extracted${NC}\n"
    else
    ...
    if [ ! -f "$VMLINUZ_PATH" ]; then
        VMLINUZ_PATH="/run/live/medium/boot/vmlinuz"
    fi
    
    if [ -f "$VMLINUZ_PATH" ]; then
        sudo cp "$VMLINUZ_PATH" "$MNT/boot/"
